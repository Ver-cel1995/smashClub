...
/**
 * Активное автообъявление тренера (последнее pinned auto-post, ещё не истёкшее)
 */
export async function getActiveCoachAnnouncement(): Promise<
...
/**
 * Ближайший активный выезд + участники
 */
export async function getNextTripWithParticipants(
    userId: string
 ...
 * Ремонт для игрока
 */
export async function getRepairForPlayer(
    userId: string
): Promise<RepairSummaryPlayer | null> {
    const supabase = await createClient()
...
/**
 * Ремонт для тренера — агрегация по игрокам
 */
export async function getRepairForCoach(): Promise<RepairSummaryCoach | null> {
    const supabase = await createClient()
...
/**
 * Последний закреплённый пост ленты (не auto, не trip)
**/
export async function getHomeFeedPost(): Promise<
    (Post & { author: Pick<Profile, 'id' | 'full_name' | 'avatar_url'> }) | null
> {
    const supabase = await createClient()
...
/**
 * Ближайшая ОТМЕНЁННАЯ/специальная тренировка (в течение ближайших 3 дней) — для верхнего блока-заметки
 */
export async function getUpcomingAffectedTraining() {
    const supabase = await createClient()
    const today = new Date().toISOString().split('T')[0]
...
/**
 * Ближайшая НОРМАЛЬНАЯ (или substitute/no_coach_open) тренировка, куда игрок может прийти
 * — то есть НЕ cancelled и НЕ holiday
 */
export async function getNextTrainingWithAttendance(
    userId: string
): Promise<TrainingWithAttendance | null> {
    const supabase = await createClient()
    const today = new Date().toISOString().split('T')[0]
    ...
    `)
        .gte('date', today)
        .not('status', 'in', '(cancelled,holiday)') // <-- пропускаем отменённые и праздники
        .order('date', { ascending: true })
        .order('start_time', { ascending: true })
        .limit(1)
        .maybeSingle()
